"""Contextual QNN reproduction scaffold.

This module implements a simulator-friendly version of the Contextual QNN
paper setup:

- binary quantization d=2
- context length T=2 by default
- forecast horizon tau=1
- fidelity-style basis-state loss
- SPSA-style parameter updates
- optional asset-specific parameters for a small QMTL experiment

The ansatz is intentionally modest and documented as a reproduction scaffold,
because the paper describes the training framework and ansatz families more
fully than exact executable gate parameters.
"""
...
class ContextualQNN:
    """Small statevector Contextual QNN with SPSA training."""

    def __init__(self, config: ContextualQNNConfig = ContextualQNNConfig()):
        if config.horizon != 1:
            raise ValueError("This first reproduction scaffold supports tau=1.")
    ...
    def _state(self, context: np.ndarray, asset_id: int = 0) -> np.ndarray:
        ...
        bits = np.concatenate([context, np.zeros(self.config.horizon, dtype=int)])
    ...
    def probability_up(self, context: np.ndarray, asset_id: int = 0, shots: int | None = None) -> float:
        """Return P(next movement is positive)."""
        ...
        if shot_count:
            return float(self.rng.binomial(shot_count, p_up) / shot_count)
        return p_up

    def predict_distribution(self, context: np.ndarray, asset_id: int = 0) -> np.ndarray:
        """Return the binary continuation distribution [P(down), P(up)]."""

        p_up = self.probability_up(context, asset_id)
        return np.asarray([1.0 - p_up, p_up], dtype=float)
    ...
    def predict_proba(self, contexts: np.ndarray, asset_ids: np.ndarray | None = None) -> np.ndarray:
        ...
        return np.asarray(
            [self.probability_up(context, int(asset)) for context, asset in zip(contexts, asset_ids)],
            dtype=float,
        )

    def predict(self, contexts: np.ndarray, asset_ids: np.ndarray | None = None) -> np.ndarray:
        return (self.predict_proba(contexts, asset_ids) >= 0.5).astype(int)
    ...
    def fidelity_loss(
        ...
        """Distribution fidelity loss over empirical P(target | context)."""
        ...
        for asset_id in sorted(set(asset_ids.tolist())):
            ...
            for context in sorted({tuple(row.tolist()) for row in asset_contexts}):
                ...
                target_p_up = float(asset_targets[context_mask].mean())
                target_dist = np.asarray([1.0 - target_p_up, target_p_up], dtype=float)
                pred_dist = self.predict_distribution(context_array, int(asset_id))
                fidelity = float(np.square(np.sqrt(target_dist[0] * pred_dist[0]) + np.sqrt(target_dist[1] * pred_dist[1])))
    ...
    def spsa_step(
        ...
        """Apply one SPSA update and return the pre-update loss."""
...
def make_qmtl_asset_ids(sample_counts: list[int]) -> np.ndarray:
    """Create label ids for concatenated multi-asset context samples."""

    ids = []
    for asset_id, count in enumerate(sample_counts):
        ids.extend([asset_id] * int(count))
